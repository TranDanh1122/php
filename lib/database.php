...
class Database
{
    ...
    {
        $this->link = mysqli_connect("localhost:3306", "root", "", "cuoiky") or die();
    ...
    public function fetchSum($query)
    {
        $result = mysqli_query($this->link, $query) or die("Lỗi  truy vấn fetchSum " . mysqli_error($this->link));
        $data;
        if ($result) {
            while ($num = mysqli_fetch_assoc($result)) {
                $data = $num;
